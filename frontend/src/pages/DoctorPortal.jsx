...
import { requestAccess, addRecord, getApprovedPatients, getPatientRecords, editRecord, deleteRecord, getEmergencyProfile } from '../services/api'
...
export default function DoctorPortal() {
  ...
  const handleSelectPatient = async (patient) => {
    ...
    try {
      const [recRes, profRes] = await Promise.all([
        ...
        getEmergencyProfile(patient.health_id).catch(() => ({ data: null }))
      ])
      setPatientRecords(recRes.data)
      setSelectedPatientProfile(profRes.data)
      setTimeout(() => {
        document.getElementById(`records-${patient.patient_id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      }, 100)
  ...
  const handleSubmitCard = async (card) => {
    ...
    try {
      ...
      if (selectedPatient && selectedPatient.patient_id === parseInt(selectedPatientId)) {
        ...
        <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-8 animate-[fadeIn_0.5s_ease-out_forwards]">
            ...
            <div className="relative z-10 w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="bg-teal-500 text-white text-xs font-extrabold px-3 py-1 rounded-full uppercase tracking-widest shadow-md">Doctor</span>
                </div>
              ...
              <div className="bg-white rounded-3xl shadow-xl p-8 sm:p-12 border border-slate-100 max-w-lg w-full text-center hover:shadow-2xl transition">
                ...
                <div className="space-y-6">
                  ...
                  {accessStatus === 'success' && (
                    <div className="bg-green-50 border border-green-200 text-green-700 text-sm font-bold rounded-xl p-4 flex items-center gap-3">
                  ...
                  {accessStatus === 'error' && (
                    <div className="bg-red-50 border border-red-200 text-red-600 text-sm font-bold rounded-xl p-4 flex items-center gap-3">
                ...
                <>
                  ...
                  <div className="grid gap-5">
                    {filteredPatients.map(patient => (
                      <div key={patient.patient_id} className={`bg-white rounded-3xl shadow-sm overflow-hidden transition-all hover:shadow-md ${selectedPatient?.patient_id === patient.patient_id ? 'border-2 border-teal-500 ring-2 ring-teal-500/20' : 'border border-slate-100'}`}>
                        <div
                        ...
                        {selectedPatient?.patient_id === patient.patient_id && (
                          <div id={`records-${patient.patient_id}`} className="border-t border-slate-100 bg-slate-50 p-6 sm:p-8 animate-[fadeIn_0.3s_ease-out_forwards]">
                            {/* Patient Profile Card */}
                            {selectedPatientProfile && (selectedPatientProfile.blood_group && selectedPatientProfile.blood_group !== 'Not specified' || selectedPatientProfile.allergies && selectedPatientProfile.allergies !== 'None recorded' || selectedPatientProfile.emergency_contact_name) ? (
                              <div className="bg-slate-800 text-white p-5 rounded-2xl mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-lg border border-slate-700">
                                <div className="flex items-center gap-3">
                                  <div className="p-2 bg-slate-700 rounded-lg text-teal-400">
                                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>
                                  </div>
                                  <div>
                                    <h4 className="font-bold text-sm tracking-widest uppercase text-slate-300">Patient Profile</h4>
                                    <p className="text-lg font-extrabold">{selectedPatientProfile.name}</p>
                                  </div>
                                </div>
                                <div className="flex flex-wrap items-start sm:items-center gap-4 w-full sm:w-auto mt-4 sm:mt-0">
                                  <div className="flex flex-col gap-1">
                                    {selectedPatientProfile.blood_group && selectedPatientProfile.blood_group !== 'Not specified' ? (
                                      <span className="text-xs font-extrabold px-3 py-1.5 rounded-full bg-red-600 text-white shadow-sm w-fit uppercase tracking-widest">
                                        BLOOD: {selectedPatientProfile.blood_group}
                                      </span>
                                    ) : (
                                      <span className="text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-slate-700 text-slate-300 border border-slate-600 w-fit">
                                        Blood: Unknown
                                      </span>
                                    )}
                                  </div>
                                  <div className="flex flex-col gap-1 w-full sm:w-auto">
                                    {selectedPatientProfile.allergies && selectedPatientProfile.allergies !== 'None recorded' ? (
                                      <div className="flex flex-col gap-1.5">
                                        <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-md bg-amber-500/90 text-white shadow-sm w-fit flex items-center gap-1.5">
                                          ⚠ ALLERGIES
                                        </span>
                                        <span className="text-sm font-semibold text-white bg-slate-900/50 px-3 py-2 rounded-lg border border-slate-700 max-w-sm whitespace-pre-wrap">{selectedPatientProfile.allergies}</span>
                                      </div>
                                    ) : (
                                      <span className="text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full bg-green-500/20 text-green-300 border border-green-500/30 w-fit">
                                        No Known Allergies
                                      </span>
                                    )}
                                  </div>
                                  <div className="w-full sm:w-auto sm:ml-auto border-t sm:border-t-0 sm:border-l border-slate-700 pt-3 sm:pt-0 sm:pl-5 flex flex-col justify-center">
                                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Emergency Contact</p>
                                    <span className="text-base font-extrabold text-white">{selectedPatientProfile.emergency_contact_name || 'Not provided'}</span>
                                    {selectedPatientProfile.emergency_contact_phone && (
                                      <span className="text-sm font-bold text-teal-400 flex items-center gap-1 mt-0.5">
                              ...
                              <div className="bg-slate-800 text-slate-400 p-5 rounded-2xl mb-8 flex items-center gap-3 shadow-lg border border-slate-700">
                                <svg className="w-6 h-6 text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                <span className="font-bold text-sm">Patient hasn't filled their profile yet</span>
                              </div>
                            )}
                            ...
                            {loadingRecords ? (
                              ...
                              <div className="text-center py-10 bg-white rounded-2xl border border-slate-200 flex flex-col items-center">
                                <p className="text-slate-500 text-base font-bold mb-4">No records yet for this patient</p>
                                <button
                                  onClick={() => { setSelectedPatientId(patient.patient_id); setActiveTab('addrecord') }}
                                  className="text-sm font-bold bg-teal-600 text-white px-5 py-2.5 rounded-xl hover:bg-teal-700 transition shadow-sm flex items-center gap-2 uppercase tracking-wide"
                                >
                                  + Add Record
                                </button>
                              </div>
                                      ...
                                      <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                        <div className="flex-1 w-full">
                                          <div className="flex flex-wrap items-center gap-3 mb-3">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-wider text-white px-3 py-1 rounded-full shadow-sm" style={{ backgroundColor: getRecordColor(r.record_type) }}>
                                              {r.record_type}
                                            </span>
                                          </div>
                                          ...
                                          {r.file_urls && r.file_urls.length > 0 && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                              {r.file_urls.map((url, i) => (
                                                <a key={i} href={url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs font-bold bg-white border border-slate-200 text-slate-700 px-4 py-2 rounded-xl hover:bg-slate-50 transition shadow-sm">
                                        ...
                                        <div className="flex gap-2 sm:flex-col lg:flex-row mt-4 sm:mt-0">
          ...
          {activeTab === 'addrecord' && (
            <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-6 relative z-10">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-11 pr-4 py-4 bg-white/10 border border-white/20 rounded-2xl focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm font-semibold text-white placeholder-slate-400 transition-all"
                      value={patientSearch}
                      onChange={e => setPatientSearch(e.target.value)}
                    />
                  </div>
              ...
              <div className="space-y-6 lg:ml-8 lg:border-l-2 lg:border-dashed lg:border-slate-200 lg:pl-8 py-4">
                {recordCards.map((card, index) => (
                  <div key={card.id} className={`bg-white rounded-3xl shadow-lg border p-6 sm:p-10 relative transition-all ${card.status === 'success' ? 'border-green-200' : 'border-slate-200'}`}>
                    <div className="flex justify-between items-center mb-6">
                      ...
                      {recordCards.length > 1 && (
                        <button onClick={() => setRecordCards(prev => prev.filter(c => c.id !== card.id))} className="text-xs font-bold text-red-500 hover:text-white bg-red-50 hover:bg-red-500 px-4 py-2 rounded-xl transition uppercase tracking-wide">
                          Remove Entry
                        </button>
                      )}
                    </div>

                    {card.status === 'success' && <div className="bg-green-50 border border-green-200 text-green-700 text-sm font-bold rounded-xl px-5 py-4 mb-6 flex items-center gap-3"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg> Record saved successfully!</div>}
                    {card.status === 'error' && <div className="bg-red-50 border border-red-200 text-red-600 text-sm font-bold rounded-xl px-5 py-4 mb-6 flex items-center gap-3"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12"/></svg> Failed to save. Please try again.</div>}
                    {card.status === 'error-fields' && <div className="bg-amber-50 border border-amber-200 text-amber-700 text-sm font-bold rounded-xl px-5 py-4 mb-6 flex items-center gap-3"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg> Ensure a patient is selected and title is provided.</div>}
                    ...
                    <div className="grid sm:grid-cols-2 gap-6 mb-8">
                      ...
                      <div>
                        ...
                        {card.record_type === 'Other' && (
                          <input type="text" placeholder="Specify custom category..." className="w-full border border-slate-200 bg-slate-50 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 transition-all shadow-sm mt-3" value={card.customType} onChange={e => updateCard(card.id, 'customType', e.target.value)} disabled={card.status === 'success'} />
                        )}
                      </div>
                      ...
                      <div className="sm:col-span-2">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                            {card.files.map((file, fi) => (
                              <div key={fi} className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3 shadow-sm">
                                ...
                                {card.status !== 'success' && (
                                  <button onClick={() => removeFileFromCard(card.id, fi)} className="text-red-400 hover:text-red-600 p-2 hover:bg-red-50 rounded-lg transition">
                        ...
                        {card.status !== 'success' && (
                          <label className="relative flex flex-col items-center justify-center w-full py-10 border-2 border-dashed border-slate-300 bg-slate-50/50 rounded-2xl hover:bg-teal-50 cursor-pointer transition-colors group">
